...
RESPONSES_DIR = os.path.join("generate_responses", "responses")
EVAL_DIR = os.path.join("generate_responses", "evaluated")
CACHE_FILE = os.path.join("generate_responses", "factcheck_cache.json")
...
for model in MODELS:
    in_path = os.path.join(RESPONSES_DIR, f"{model}.csv")
    out_path = os.path.join(EVAL_DIR, f"{model}_hallucination_scores.csv")
    ...
    print(f"Evaluating: {model} ({len(df)} prompts)")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"Scoring {model}"):
        g = row["ground_truth"]
        r = row["llm_response"]
